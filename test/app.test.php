...
class AppTest extends UnitTestCase
{
    public function testCallClass()
    {
        ...
        $this->assertEqual('Hello!', $contents,
            'Foi chamada a classe BufferConstructTest. %s');
    }
    ...
    public function testCallGetPostAny()
    {
        ...
        $contents = get_content();
        $this->assertEqual('get', $contents,
            'Foi chamado o metodo get pelo REQUEST. %s');

        ob_start(); app(array('.*' => 'BufferMethodTest'), '', 'POST');
        $contents = get_content();
        $this->assertEqual('post', $contents,
            'Foi chamado o metodo post pelo REQUEST. %s');

        ob_start(); app(array('.*' => 'BufferMethodTest'), '', 'ANY');
        $contents = get_content();
        $this->assertEqual('any', $contents,
            'Foi chamado o metodo any pelo REQUEST. %s');
    }
    ...
    public function testError()
    {
        ...
        $contents = get_content();
        $this->assertEqual("Classe Erro\n404 - Page Not Found", $contents, 
            'A url nao foi encontrada na lista desejada. %s');
    }
    ...
    public function testArgs()
    {
        ...
        $contents = get_content();
        $this->assertEqual("Called get\n".var_export(array('post', '25'), true),
            $contents, 'Deveria retornar o metodo com os argumentos. %s');
    }
